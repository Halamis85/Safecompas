    ...
    public function store(StoreObjednavkaRequest $request): JsonResponse
    {
       ...
       /* $admins = User::where('role', 'admin')->get();
        foreach ($admins as $admin) {
            $admin->notify(new OrderCreated($objednavka));
        }*/
        // Nebo jen konkrétnímu uživateli
            User::find(1)->notify(new OrderCreatedNotification($objednavka));

            if ($userId) {
                event(new OrderCreatedEvent($userId, $request->validated()));
            }
